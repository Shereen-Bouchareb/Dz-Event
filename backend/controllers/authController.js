...
exports.signupPrestataire = async (req, res) => {
  ...
  try {
    ...
    console.log(email);
    const existingUser = await userQueries.checkUserExists(
      email,
      "prestataire"
    );
    ...
    await userQueries.insertPrestataire(
      email,
      hashedPassword,
      firstname,
      familyname,
...
exports.signupClient = async (req, res) => {
  ...
  try {
    ...
    await userQueries.insertClient(
      email,
      hashedPassword,
      firstName,
      familyName
  ...
  try {
    if (userType === "prestataire") {
      ...
      if (userType === "client") {
        ...
        return res.status(401).json({ error: "userType require" });
      }
    }
    ...
    const token = jwt.sign(
      { id: user.Prestataire_id || user.client_id, userType: userType },
