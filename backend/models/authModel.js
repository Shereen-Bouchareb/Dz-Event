...
const insertPrestataire = async (email, hashedPassword, firstname, familyname, userBio, jobDescription, role) => {
    const query = "INSERT INTO prestataire (Gmail_ad, password, firstname, familyname, userBio, Job_description, role) VALUES (?, ?, ?, ?, ?, ?, ?)";
    try {
        await db.query(query, [email, hashedPassword, firstname, familyname, userBio, jobDescription, role]);
    } catch (error) {
        throw new Error("Database query failed: " + error.message);
    }
};
